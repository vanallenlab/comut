...
def parse_gatktotable_vcf(gatk_table_vcf, variants=None, rename=None):
    '''Parses a tab delimited table from GATK's VariantsToTable into the
    form CoMut expects.

    Params:
    -------
    gatk_table_vcf: pandas dataframe
        Tab delimited output of GATK's VariantsToTable

    variants: list-like
        A list of variants to subset the table to. Defaults to
        all frameshift_variants, all inframe_deletions, all inframe insertions,
        all missense_variants, and all splice_variants

    rename: dict
        A dictionary to rename values in the MAF. Defaults to
        collapsing insertions and deletions to indel and all specific variant
    ...
    # default to nonsilent variants
    if variants is None:
        ['Nonsense_Mutation', 'In_Frame_Del', 'Frame_Shift_Ins', 'Splice_Site', 'In_Frame_Ins', 'Frame_Shift_Del', 'Missense_Mutation']
    ...
    # subset maf to relevant mutation types
    parsed_maf = subset_maf_df[subset_maf_df['value'].isin(variants)]

    # rename variants
    parsed_maf['value'] = parsed_maf['value'].replace(rename).copy()
